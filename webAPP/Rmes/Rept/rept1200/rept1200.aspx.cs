...
namespace Rmes.WebApp.Rmes.Rept.rept1200
{
    public partial class rept1200 : BasePage
    {
        ...
        protected void ASPxGridView1_CustomCallback(object sender, DevExpress.Web.ASPxGridView.ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            + " SELECT a.SN,a.PLAN_SO,a.START_TIME,a.START_TIME,c.STATION_NAME,a.PRODUCT_MODEL,b.user_name,d.shift_name,e.team_name,'" + MachineName + "',a.PLAN_CODE "
            ...
            if (FUNC1 == "BC")
            {
                string RQW = ASPxDateEdit1.Date.ToShortDateString();
                string RQ = ASPxDateEdit1.Text.ToString().Substring(0,10);
                 
                if (BC1 == "全部" || BC1 == "")
                {
                    InSql = InSql + " and to_date(to_char(START_TIME,'yyyy-mm-dd'),'yyyy-mm-dd')=to_date('" + ASPxDateEdit1.Date.ToShortDateString() + "','yyyy-mm-dd')";
                }
                else
                {
                    InSql = InSql + " and to_date(to_char(START_TIME,'yyyy-mm-dd'),'yyyy-mm-dd')=to_date('" + ASPxDateEdit1.Date.ToShortDateString() + "','yyyy-mm-dd') and d.shift_name='" + BC1 + "'";
                }
                if (ZD1 == "全部" || ZD1 == "")
                {
            ...
            {
                InSql = InSql + " and START_TIME>=to_date('" + ASPxDateEdit1.Text.Trim() + "','yyyy-mm-dd hh24:mi:ss') and START_TIME<=to_date('" + ASPxDateEdit2.Text.Trim() + "','yyyy-mm-dd hh24:mi:ss') ";
                if (ZD1 == "全部" || ZD1 == "")
                {
                    InSql = InSql + " and a.pline_code='" + GZDD1 + "' ";
                }
